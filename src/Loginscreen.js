...
class Loginscreen extends Component {
  constructor(props){
    ...
    loginButtons.push(
      ...
      <MuiThemeProvider>
        <div>
           <RaisedButton label={"ثبت نام به عنوان بیمار"} primary={true} style={style} onClick={(event) => this.handleClick(event,'student')}/>
       </div>
       </MuiThemeProvider>
       <MuiThemeProvider>
       <div>
          <RaisedButton label={"ثبت نام به عنوان پزشک"} primary={true} style={style} onClick={(event) => this.handleClick(event,'teacher')}/>
      </div>
      </MuiThemeProvider>
      </div>
    )
  ...
  componentWillMount(){
    ...
    var loginmessage = "هنوز ثبت نام نکرده اید، اکنون ثبت نام کنید";
    this.setState({
  ...
  handleClick(event,userRole){
    ...
    if(this.state.isLogin){
      ...
      loginmessage = "قبلاً ثبت نام کرده اید، وارد شوید";
      let loginButtons=[];
      loginButtons.push(
      ...
      loginmessage = "هنوز ثبت نام نکرده اید، اکنون ثبت نام کنید";
      this.setState({
                     loginscreen:loginscreen,
                     loginmessage:loginmessage,
                     loginButtons:loginButtons,
